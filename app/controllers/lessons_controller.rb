class LessonsController < ApplicationController
  def new
    ...
    @learned = Lesson.joins("JOIN categories ON lessons.category_id like categories.id")
    ...
    @not_learned = Category.joins("LEFT OUTER JOIN lessons ON categories.id = lessons.category_id LEFT OUTER JOIN words ON words.category_id like categories.id")
                    ...
                    .paginate(page: params[:page], per_page: 2 ).order('created_at DESC')
    @coming = Category.joins("LEFT OUTER JOIN lessons ON categories.id = lessons.category_id LEFT OUTER JOIN words ON words.category_id like categories.id")
                      .select("categories.*")
                      .where("isnull(lessons.id) AND isnull(words.id)")
                      .paginate(page: params[:page], per_page: 2 ).order('created_at DESC')
